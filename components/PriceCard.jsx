...
const PriceCard = ({ name, price, features, text, dark, btnText }) => {
  return (
    ...
    {dark ? <div className="price-card flex flex-col overflow-hidden rounded-3xl p-6 shadow-lg shadow-gray-900/5 bg-neutral-900">
        <h3 className="text-md font-semibold leading-6 text-white">{name}</h3>
        <p className="mt-5 text-[32px] tracking-tight text-white">${price}</p>
        ...
        <h3 className="text-md leading-6 font-semibold text-neutral-900">{name}</h3>
        <p className="mt-5 text-[32px] tracking-tight text-neutral-900">${price}</p>
